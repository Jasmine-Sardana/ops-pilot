...
class TestFixAgentDescribe:
    def test_describe_returns_string(self):
        agent = FixAgent(backend=MagicMock())
...
class TestFixAgentRun:
    def _make_agent(self, mock_backend, pr_response: dict | None = None) -> FixAgent:
        ...
        mock_backend.complete.return_value = json.dumps(pr_response)
        return FixAgent(backend=mock_backend, demo_mode=True)
    ...
    def test_run_sets_status_failed_on_error(
        ...
        agent = FixAgent(backend=mock_backend, demo_mode=True)
    ...
    def test_run_raises_on_invalid_json_response(
        ...
        mock_backend.complete.return_value = "plain text, not json"
        agent = FixAgent(backend=mock_backend, demo_mode=True)
